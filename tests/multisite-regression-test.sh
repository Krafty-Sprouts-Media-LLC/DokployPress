...
assert_file_contains "${ROOT_DIR}/wordpress/docker-entrypoint-custom.sh" 'write_wp_config_block "WORDPRESS_MULTISITE_CONFIG"' "Entrypoint writes managed multisite block"
...
assert_file_contains "${ROOT_DIR}/docker-compose.yml" 'WORDPRESS_CONFIG_EXTRA_PERSISTENT=${WORDPRESS_CONFIG_EXTRA_PERSISTENT:-}' "Main compose exposes persistent config extra env"
